...
#
# ```
# docker network create django_e2e_benchmarks_net
# docker image pull --platform linux/amd64 <registry>/<image>:<tag>
#
# docker run --detach --name django_e2e_benchmarks \
#     --user root \
#     --env-file /etc/django_e2e_benchmarks/.env \
#     --env PORT=8000 \
#     --network django_e2e_benchmarks_net \
#     --publish 8000:8000 \
#     --volume django_e2e_benchmarks_static:/app/static \
#     <registry>/<image>:<tag>
# ```
#
...
CMD ["sh", "-c", "gunicorn --bind 0.0.0.0:$PORT django_e2e_benchmarks.wsgi:application"]
