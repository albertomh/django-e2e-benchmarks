# django_e2e_benchmarks developer README

## Development prerequisites

In addition to the [Prerequisites](../README.md#prerequisites) listed in the main README
you will need the following to develop `django_e2e_benchmarks`:

- [pre-commit](https://pre-commit.com/)

## The djereo project template

`django_e2e_benchmarks` has been generated using the [djereo](https://github.com/albertomh/djereo)
project template.

To update `django_e2e_benchmarks` to a newer version of `djereo`:

```sh
cd ~/Projects/django_e2e_benchmarks/
...
`django_e2e_benchmarks/__init__.py` exposes project metadata such as version and author information.
To use it in a view:
...
def view(request):
    return HttpResponse(f"django_e2e_benchmarks v{__version__}")
```
