...
class TestDevModeCheck(SimpleTestCase):
    ...
    def test_fail_not_dev_mode(self):
        ...
        self.assertEqual(result[0].id, "django_e2e_benchmarks.W001")


@isolate_apps("django_e2e_benchmarks", attr_name="apps")
class CheckModelNamesTests(SimpleTestCase):
    def test_success(self):
        class Single(models.Model):
            ...
            class Meta:
                app_label = "django_e2e_benchmarks"
    ...
    def test_fail(self):
        ...
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0].id, "django_e2e_benchmarks.E001")
        self.assertEqual(
            result[0].hint,
            (
                "Rename to the singular form, like “Plural”, or mark the name "
                "as allowed by adding 'Plurals' to "
                "django_e2e_benchmarks.checks.SAFE_MODEL_NAMES."
            ),
        )
